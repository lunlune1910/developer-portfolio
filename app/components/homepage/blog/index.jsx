  ...
  return (
    <div id='blogs' className="relative z-50 border-t my-12 lg:my-24 border-[#25213b]">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 md:gap-5 lg:gap-8 xl:gap-10">
        ...
        {/* Then dev.to posts */}
        {blogs.slice(0, Math.max(0, 6 - localPosts.length)).map((blog, i) => (
          blog?.cover_image &&
          <FadeInSection key={`devto-${i}`} direction="fadeUp" delay={(localPosts.length + i) * 0.1}>
            <BlogCard blog={blog} />
          </FadeInSection>
        ))}
      </div>

      <FadeInSection direction="fadeUp" delay={0.3}>
